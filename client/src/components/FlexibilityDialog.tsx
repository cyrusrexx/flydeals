...
export default function FlexibilityDialog({ travelClass, open, onOpenChange }: Props) {
  const data = useMemo(() => analyzeFlexibility(travelClass), [travelClass]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" data-testid="flex-dialog">
        ...
        <div className="space-y-4">
          <Badge className={`text-sm px-3 py-1 ${riskColor}`}>Risk: {data.riskScore}</Badge>

          <div className="p-3 rounded-lg bg-secondary/30">
            <div className="flex items-center gap-2 mb-1.5">
              {data.changePolicy.fee === 0 ? <CheckCircle2 className="h-4 w-4 text-emerald-500" /> : <AlertTriangle className="h-4 w-4 text-amber-500" />}
              <p className="text-sm font-medium">Change Policy</p>
              <span className="text-sm font-semibold tabular-nums ml-auto">{data.changePolicy.fee === 0 ? "Free" : `$${data.changePolicy.fee} fee`}</span>
            </div>
          ...
          <div className="p-3 rounded-lg bg-secondary/30">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <Badge variant="secondary" className="text-xs ml-auto">{data.cancelPolicy.refund}</Badge>
            </div>
            ...
            <ul className="space-y-1.5">
              {data.hiddenClauses.map((clause, i) => (
                <li key={i} className="flex items-start gap-2 text-xs text-muted-foreground">
                  <span className="text-primary mt-0.5">•</span>{clause}
                </li>
              ))}
            </ul>
          </div>
